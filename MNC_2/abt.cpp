 ...
 * or lost, according to user-defined probabilitiesthr
...
int calc_checksum(int sequence_no, int acknowledgement_no, char* payload_to_add) {
    int sum=0;
...
void A_output(struct msg message) //ram's comment - students can change the return type of the function from struct to pointers if necessary
{
    ...
    if(in_transit==1)
    {
        printf("message still in transit...Discard Packet\n");
        return;
    }
    ...
    printf("packet_to_send.payload %s\n", packet_to_send.payload);
    seq_no=1-seq_no;
    packet_to_send.seqnum=seq_no;
    packet_to_send.acknum=seq_no;
    packet_to_send.checksum=calc_checksum(packet_to_send.seqnum, packet_to_send.acknum, packet_to_send.payload);
    printf("Seq No : %d Ack No : %d Checksum : %d\n", packet_to_send.seqnum, packet_to_send.acknum, packet_to_send.checksum);
    in_transit=1;
    number_2++;
...
void A_input(struct pkt packet) {
    ...
    int check_for_ack_error=0;
    check_for_ack_error=calc_checksum(packet.seqnum, packet.acknum, packet.payload);
    printf("check_for_ack_error %d , recv_checksum %d \n", check_for_ack_error, packet.checksum);

    //Desired packet with no corruption
    if(seq_no==packet.acknum && check_for_ack_error==packet.checksum) {
        printf("Desired ACK Received with No Corruption\n");
        stoptimer(0);
        struct pkt ack_pck_received;
        ack_pck_received=packet;
        printf("ack_pck_received.payload %s\n", ack_pck_received.payload);
        in_transit=0;
    }
    else
        //Undesired Packet
    {if(check_for_ack_error!=packet.checksum) {
         //Retransmission
         printf("Acknowledgement is Corrupted\nNeed to retransmit\n");
     }
     else if (seq_no!=packet.acknum) {
         printf("Non Desired ACK\n");
...
void B_input(struct pkt packet) {
    ...
    check_for_error=calc_checksum(packet.seqnum, packet.acknum, packet.payload);
    printf("Packet: Seq_No %d, Ack_No %d, Payload %s\n", packet.seqnum, packet.acknum, packet.payload);

    printf("Checksum %d Check_for_error %d\n", packet.checksum, check_for_error);
    if(b_wait_for==packet.acknum && check_for_error==packet.checksum) {
        printf("Desired Packet with No Corruption\n");
        ...
        ack_packet.acknum = b_wait_for;
        ack_packet.checksum=calc_checksum(ack_packet.seqnum, ack_packet.acknum, ack_packet.payload);
        tolayer3(1, ack_packet);
        b_wait_for = 1 - b_wait_for;
    }
    else {//Undesired Packet
        if(check_for_error!=packet.checksum) {
            printf("Corrupted Packet Received\nNo Acknowledgement Sent\n");
            return;
        }
        else if(b_wait_for!=packet.acknum) {
            printf("Non Desired Packet\nSending Acknowledgement");
            struct pkt ack_packet=packet;
            ack_packet.acknum = 1 - b_wait_for;
            ack_packet.checksum=calc_checksum(ack_packet.seqnum, ack_packet.acknum, ack_packet.payload);
            tolayer3(1, ack_packet);
        }
    }
